...
import com.cts.repository.UserRepository;          // ← ADD
...
public class DriverServiceImpl implements DriverService {
    ...
    public DriverResponse createDriver(CreateDriverRequest request) {
        ...
        Driver existing = driverRepository.findByEmail(request.getEmail());
        if (existing != null) {
        ...
        // 3. Create User and save to users table first ← KEY FIX
        ...
        userRepository.save(user);  // ← saves to users table

        // 4. Create Driver with encoded password
    ...
    public Driver deleteDriver(Long id) {
        Driver existing = driverRepository.findByDriverId(id);
        if (existing == null) {
            throw new NotExistException("Driver not found with id: " + id);
        }
    ...
    public DriverResponse updateDriver(Long id, UpdateDriverRequest request) {
        Driver driver = driverRepository.findById(id)
                .orElseThrow(() -> new NotExistException("Driver not found with id: " + id));
        ...
        if (request.getSex()              != null) driver.setSex(request.getSex());
